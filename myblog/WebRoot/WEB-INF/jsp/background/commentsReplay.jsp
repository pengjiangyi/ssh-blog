					...
					<c:forEach items="${list}" var="rep">
						<tr class="text-c">
							...
							<td class="f-14 td-manage"> <a style="text-decoration:none" class="ml-5"
								onClick="article_edit('资讯编辑','${rep.text}','${rep.id}')"
								href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i>
							</a> <a style="text-decoration:none" class="ml-5"
								onClick="article_del(this,${rep.id})" href="javascript:;"
...
function datadel()
	...
	$.ajax({
			type: 'POST',
			url: rootPath+'/deleteCommentsReplayALL.ajax',
			data:{
			list:list,
			list2:list2
			},
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: rootPath+'/deleteCommentsReplayById.ajax',
			data:{
			ID:id
			},
			success: function(data){
...
function article_edit(obj,text,id){
		layer.open({
				...
				type : 2,
				area : [ "300px", "200px" ],
				content : "/myblog/updateCommentsReplay-"+id+"-"+text+".html"
			});
