...
<body>
	...
	<div class="page-container">
		<div class="text-c">
		<form action="findWords-like.html" method="post">
					...
					<c:forEach items="${list}" var="words">
						<tr class="text-c">
							...
							<td><c:forEach items="${list2}" var="rep">
									<c:if test="${rep.words.id==words.id}">
						${rep.text}</br>
									</c:if>
								</c:forEach>
							</td>
							...
							<td class="td-status"><c:if test="${words.status=='已回复'}">
									<span class="label label-success radius">${words.status}
									</span>
								</c:if> <c:if test="${words.status=='未回复'}">
									<span class="label label-error radius">${words.status} </span>
								</c:if>
							</td>
							<td class="f-14 td-manage"><a style="text-decoration:none"
								onClick="article_reply(this,${words.id})" href="javascript:;"
								title="回复"><i class="Hui-iconfont">&#xe6de;</i> </a> <a
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: rootPath+'/deleteWordsById.ajax',
			data:{
			ID:id
			},
...
function article_reply(obj,id){
		var mail=$(obj).parent().prev().prev().prev().prev().prev().text();
		 ...
		 layer.open({
				...
				type : 2,
				area : [ "300px", "200px" ],
				content : "/myblog/wordsReplay-"+id+"-"+mail+".html"
			});
	...
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: rootPath+'/deleteWordsall.ajax',
			traditional:true,
			data:{
